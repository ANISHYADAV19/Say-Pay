...
export default function TypedInputFallback({ onSubmit, promoted = false, disabled = false }) {
  ...
  return (
    <form onSubmit={submit} className="flex items-center gap-2" aria-label="Type a command">
      <input
        type="text"
        value={value}
        onChange={(e) => setValue(e.target.value)}
        disabled={disabled}
        placeholder={promoted ? 'Type a command, e.g. “add 2 apples”' : 'Or type a command…'}
